...
describe("view feedback", () => {
    it("tests view feedback table updates", () => {
      ...
      cy.get('[data-qa="table"]').find('tr').eq(0).find('th').each(($el, index): void  => {

        const heading: string= $el.text();

        if (heading === "Title") {
          cy.get('[data-qa="table"]').find('tr').eq(1).find('td').eq(index).then(($td): void  => {

            const title: string = $td.text();
            ...
            cy.get('[data-qa="table"]').find('tr').eq(1).find('td').eq(index).then(($td2): void  => {

              const closedTitle: string = $td2.text();
