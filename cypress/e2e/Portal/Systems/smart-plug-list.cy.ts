    ...
    it("tests my smart plugs page", () => {
        ...
        cy.get('[data-qa="table"]').find('tr').eq(0).find('th').each(($elm, index): void  => {
            ...
            if (text1 === "Deleted At") {
                ...
                cy.get('[data-qa="table"]').find('tr').eq(tableDataIndex).find('td').eq(index).then(($td): void  => {

                    const text2: string = $td.text();
                    console.log(text2)
                    const result: boolean = dateAndTime.test(text2);
                    console.log(result)
                    if (!result) {
        ...
        cy.get('div[class="v-select__slot"]').eq(1).should('be.visible').type('CavansSmartPlugs');
