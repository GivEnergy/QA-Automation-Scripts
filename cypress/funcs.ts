export function addRNG(string: string) : string {
    let num : number = Math.floor(Math.random() * 100000); //generates a random number
    let numToString : string = num.toString();
    return string + numToString; //concatenates this num onto a passed in string
}

export function dashboardSelect(navItem : string, adminItem? : string) : void {
...
export function checkPageNav() : void { //checks all the paginate navigations work
...
export function createAccount(username : string, clear : boolean): void {
    //creates aliases
...
export function loginCheck(username : string, password : string, check : boolean) : void{
...
export function changeDetails(correct: boolean): void {
    //creates aliases
...
export function changePassword(current : string, newP : string, repeatP : string, order : string) : void {
    //creates aliases
...
export function checkReturnsFormat(): void {
...
export function checkReturnsActions(): void {
...
export function tableContains(heading : string, value : string, errorMessage : string): void {
    ...
    cy.get('[data-qa="table"]').find('tr').eq(0).find('th').each(($elm, index) => {
        ...
        if (text1 === heading) {
            ...
            const text2: string = $td.text().trim();
            console.log(text2);
            console.log(value);
            if (text2 !== value) {
...
export function tableCSS(heading : string, expectedCSS : string, errorMessage : string): void {
    //finds the index of a table header
    ...
    cy.get('[data-qa="table"]').find('tr').eq(0).find('th').each(($elm, index) => {
        ...
        if (text1 === heading) {
            ...
            cy.get('[data-qa="table"]').find('tr').eq(1).find('td').eq(index).find('i').then(($i) => {
                ...
                const actualCSS: string = $i.css('color');
...
export function tableClick(heading : string, value? : string): void {
    //finds the index of a table header
...
export function tableRegex(heading : string, regex : RegExp, errorMessage: string): void {
    //finds the index of a table header
    ...
    cy.get('[data-qa="table"]').find('tr').eq(0).find('th').each(($elm, index) => {
        ...
        if (text1 === heading) {
            ...
            const text2: string = $td.text();
            console.log(text2)
            const result: boolean = regex.test(text2);
            console.log(result)
            if (!result) {
              throw new Error(errorMessage);
            }
...
export function selectDashboardCard(title: string, description: string, search: string, user: string): void {
...
export function myInverterTab(tab: string): void {
    //clicks a specific tab on inverter info page
    cy.get('[data-qa="card.tab"]').contains(tab).click();
}

export function inverterSoftwareCheck(titles: string[], subtitles: RegExp[]): void  {
...
export function myInverterNotificationsTable(header: string[]): void  {
...
export function myInverterTable(header: string[]): void  {
...
export function checksCounterIncreasesAndDecreases(): void  {
    ...
    cy.get('[data-qa="icon.notification"]').next().find('span[class*="v-badge__badge"]').then(($span) => {
        ...
        cy.get('[data-qa="icon.notification"]').next().find('span[class*="v-badge__badge"]').then(($span) => {
          ...
          const text2: string = $span.text();
          const num2: number = Number(text2);
...
export function checkMarkAsReadWorks(withElse: boolean): void  {
...
export function updateWarrantyAndCheck(warranty: string, num: number, tableDataIndex: number): void  {
    ...
    cy.get('[data-qa="general.title"]').contains('Commission Date').next('[data-qa="general.subtitle"]').then(($div): void  => {

      const text: string = $div.text();
      const splitString: string[] = text.split('-');

      const tempNum: number = Number(splitString[0]) + num;

      splitString[0] = tempNum.toString();

      const estimate: string = splitString.join("-");
...
export function changeEnergyGraphData(type: string, dataTypes: string[]): void  {
...
export function checkWarranty(headingIndex: number, tableDataIndex: number): void  {
    ...
    cy.get('[data-qa="table"]').find('tr').eq(tableDataIndex).find('td').eq(headingIndex).then(($td) => {
        ...
        if (text === 'Standard') {
            ...
            let index: number = tableDataIndex++;
            checkWarranty(headingIndex, index);
        }
    });
}

export function checkBarChartXAxis(): void  {
...
export function checkDateRangePicker(): void  {
...
export function deleteSmartPlug(tableDataIndex: number, index: number): void  {
    cy.get('[data-qa="table"]').find('tr').eq(tableDataIndex).find('td').eq(index).find('i').then(($el) => {

        const value: string = $el.attr('class');
        ...
        if (!value.includes('mdi-delete')) {
            ...
            cy.get('[data-qa="title.dialog"]').should('be.visible');
            cy.get('[data-qa="text.dialog"]').should('be.visible');
            cy.get('[data-qa="button.action"]').contains('Confirm').click();
